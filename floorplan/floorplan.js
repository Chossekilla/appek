...
(function () {
  ...
  function attachItemHandlers(el, it) {
    ...
    let resizeOrigW, resizeOrigH;
    let rotateOrig, rotateStartAngle;
    ...
    function onDown(e) {
      e.stopPropagation();
      // Check if click on resize/rotate handle
      const tgt = e.target;
    ...
    function onMove(e) {
      e.preventDefault();
      const p = pointer(e);
      const dx = (p.x - startX) / State.zoom;
      const dy = (p.y - startY) / State.zoom;
      if (!isDragging && (Math.abs(dx) > 2 || Math.abs(dy) > 2)) isDragging = true;
      if (isDragging) {
        let nx = origX + dx;
        let ny = origY + dy;
        if (State.snap) { nx = Math.round(nx / 10) * 10; ny = Math.round(ny / 10) * 10; }
    ...
    function onUp() {
      ...
      if (isDragging) { pushHistory(); renderProps(); }
    }
    ...
    function startResize(e) {
      ...
      const p = pointer(e);
      startX = p.x; startY = p.y;
      resizeOrigW = it.w; resizeOrigH = it.h;
    ...
    function onResizeMove(e) {
      e.preventDefault();
      const p = pointer(e);
      let nw = Math.max(30, resizeOrigW + (p.x - startX) / State.zoom);
      let nh = Math.max(30, resizeOrigH + (p.y - startY) / State.zoom);
      if (State.snap) { nw = Math.round(nw / 10) * 10; nh = Math.round(nh / 10) * 10; }
      it.w = nw; it.h = nh;
    ...
    function startRotate(e) {
      ...
      function onRotateMove(ev) {
        ...
        let deg = rotateOrig + (ang - rotateStartAngle) * 180 / Math.PI;
  ...
  async function applyToProduction(silent) {
    // Save current zones+items do DB přes admin_tables.php
    // Nejdřív musíme synchronizovat zóny — POST zone_save pro každou + save_layout pro tables.
    // Pro jednoduchost: použijeme save_user_template -> apply_user_template flow.
    // Ale pro live editor: zde to děláme tak že rovnou hodíme aktuální data jako "user_template snapshot" via save_user_template,
    // pak apply_user_template (s WIPE) → DB má identický snapshot.
    // Pro úsporu DB writeu: provedeme to v jedné transakci přes nový snapshot endpoint.

    // Nejjednodušší: vytvoř dočasnou šablonu pak ji applay
    // Ale to je 2 requesty. Pro user-friendly UX: jen toast a save uložíme.

    // Pro tento prototyp: voláme apply_user_template přímo zaslat zones+items.
    // Vlastně tady použiju jednodušší cestu: každá zóna POST zone_save, pak save_layout pro items.
    // To je správnější.

    try {
      // 1) Wipe + insert all zones + items přes hromadný endpoint.
      // Použijeme: save_user_template s rezervovaným nazev → uložíme jako "__live_buffer__", potom apply.
      const dummyName = '__live_buffer_' + Date.now();
      // Hack: nejdřív musíme mít data v DB. Use admin_tables endpoint to sync.
      // Pro tento POC — Use 2-step:
      //   a) DELETE all zones + tables (přes /admin endpoints)
      //   b) Bulk insert
      // Lepší: navrhneme nový endpoint admin_tables.php?action=sync_from_editor

      // ZJEDNODUŠENÍ: Volat backend přes klasické endpointy
      // Wipe restaurant_tables a zones, pak zone_save + save_layout
      // Bohužel nemáme bulk insert pro tables — musíme přes POST per stůl.

      // V této verzi MVP: použij save_user_template + apply.
      // 1) Save current as named template
      const r = await api('admin_tables.php?action=save_user_template', {
        method: 'POST',
        body: JSON.stringify({ nazev: dummyName, popis: 'Live editor buffer', ikona: '⚡' }),
      });
      // 2) Apply it (wipes + inserts)
      await api('admin_tables.php?action=apply_user_template', {
        method: 'POST',
        body: JSON.stringify({ id: r.id, merge: false }),
      });
      // 3) Smaž dummy
      await api('admin_tables.php?action=user_template&id=' + r.id, { method: 'DELETE' });

      if (!silent) {
        toast('✓ Floor plan aplikován do produkce', 'success');
        setDirty(false);
      }
    } catch (e) {
      if (!silent) toast('Chyba: ' + e.message, 'error');
      throw e;
    }
  }
