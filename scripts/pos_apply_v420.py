#!/usr/bin/env python3
"""Apply v3.0.420 UI translation backlog (genuine remainder from audit).
...
MARK = 'v3.0.420 dopreklady UI (POS staticke UI (kasa: doklady, vratky, vouchery, dárkové karty, rozpracované))'
...
rows = json.load(open(f'{ROOT}/scripts/pos_new_v420.json', encoding='utf-8'))
entries, skipped = {}, []
for x in rows:
    cs = x['cs'].strip()
