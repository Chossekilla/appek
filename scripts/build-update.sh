...
# 🐛 v3.0.403 — vendor/.appek-version je trackovaný v gitu → deploy ho přepisoval stale
#   hodnotou (3.0.153) a hlavička vendoru po deployi lhala, dokud ho nepřepsal self-update.
echo "${VERSION}" > "$SROOT/vendor/.appek-version"
echo "🔖 Verze sjednoceny na ${VERSION}: config.php · admin.js · admin.css · admin+b2b/index.html (?v) · sw.js CACHE_VERSION (admin+b2b) · vendor/.appek-version"
