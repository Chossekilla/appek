...
if ($method === 'GET') {
    ...
    $listy = $pdo->query("
        SELECT vl.*, COUNT(vp.id) AS pocet_polozek,
               COALESCE(SUM(vp.mnozstvi_planovane), 0) AS celkem_ks
...
if ($method === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO vyrobni_list_polozky (vyrobni_list_id, vyrobek_id, mnozstvi_planovane, poznamka)
            VALUES (:vl, :v, :m, :p)
        ");
...
if ($method === 'PUT') {
    ...
    try {
        ...
        if (!empty($d['vyrobeno']) && is_array($d['vyrobeno'])) {
            $stmt = $pdo->prepare("
                UPDATE vyrobni_list_polozky SET mnozstvi_vyrobene = :m
