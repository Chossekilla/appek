...
// 🔒 v2.6.0 SECURITY FIX (C4): odstraněn hardcoded key (byl shodný ve VŠECH ZIPech!).
//    Místo toho:
//      1. POŽADUJE admin session (přihlášený admin v /admin/)
//      2. POŽADUJE per-install random check key v api/.check-install-key
...
// Pokud neexistuje per-install klíč → vygeneruj
...
// Authorization: BUĎ admin session NEBO platný per-install key (pro recovery scénář)
$authorized = ($ciUser && ($ciUser['role'] ?? '') === 'admin')
           || ($expectedKey && hash_equals($expectedKey, $providedKey));

if (!$authorized) {
    http_response_code(403);
    die('<!DOCTYPE html><html lang="cs"><head><meta charset="UTF-8"><title>403</title></head><body style="font-family:system-ui;padding:40px;text-align:center"><h1>🔒 403 Forbidden</h1><p>Vyžaduje admin session nebo per-install klíč.</p><p>Klíč najdeš jako <code>api/.check-install-key</code> přes FTP/SSH. Po použití smaž.</p></body></html>');
}

// Display errors jen pro authorized admina (neodhaluj path/secrets při 403)
